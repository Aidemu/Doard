...
switch($action)
{
    ...
    case "consulter":
	   ...
	   echo '<ul class="breadcrumb">
			...
			<li class="active">Profil de '.stripslashes($data['membre_pseudo']).'</li>
		</ul>';
       echo'<h1>Profil de '.stripslashes($data['membre_pseudo']).'</h1>';
	   ...
	   case "modifier":
		if (empty($_POST['sent'])) // Si on la variable est vide, on peut considérer qu'on est sur la page de formulaire
		{
			...
			echo '<ul class="breadcrumb">
				<li><a href="./index.php">Forum</a> <span class="divider">/</span></li>
				<li class="active">Modification du profil</li>
			</ul>';
